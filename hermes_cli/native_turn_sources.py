...
class NativeSessionView:
    ...
    def __post_init__(self) -> None:
        ...
        lineage = tuple(dict.fromkeys(str(item).strip() for item in self.compression_lineage if str(item).strip()))
...
_SESSION_BOUNDARY_LOCKS: dict[tuple[str, ...], threading.RLock] = {}
...
def register_profile_source(
    ...
    scope = str(Path(profile_home).expanduser().resolve(strict=False))
    ...
    def unregister() -> None:
        with _PROFILE_SOURCES_LOCK:
            ...
            if current.get(registration.source.name) is registration:
                _ = current.pop(registration.source.name, None)
            if not current:
                _ = _PROFILE_SOURCES.pop(scope, None)

    return unregister


def _profile_source_snapshot(profile_home: str | Path):
    scope = str(Path(profile_home).expanduser().resolve(strict=False))
    with _PROFILE_SOURCES_LOCK:
...
class NativeTurnAdmission:
    ...
    def commit(self) -> bool:
        with self._boundary_lock, self._lock:
            ...
            # Hold the admission lock through the callback so a synchronous user
            # boundary cannot race a pre-model commit in another host thread.
            try:
    ...
    *,
    ...
    for registration in tuple(registrations.values()):
        ...
        try:
            ...
            logger.warning("Native turn source %s poll failed", registration.plugin_id, exc_info=True)
            continue
        if lease is None:
            continue
        if not isinstance(lease, NativeTurnLease):
            logger.warning("Native turn source %s returned an invalid lease", registration.plugin_id)
            continue
...
def fence_registered_sources(
    ...
    for registration in tuple(registrations.values()):
        ...
        try:
            ...
            logger.warning("Native turn source %s boundary failed", registration.plugin_id, exc_info=True)


def _profile_registrations(profile_home: Path) -> Mapping[str, RegisteredNativeTurnSource]:
...
def _session_boundary_lock(session: NativeSessionView) -> threading.RLock:
    ...
    with _SESSION_BOUNDARY_LOCKS_LOCK:
        return _SESSION_BOUNDARY_LOCKS.setdefault(key, threading.RLock())
...
def fence_native_turn_sources(session: NativeSessionView, reason: str) -> None:
    ...
    with _session_boundary_lock(session):
        fence_registered_sources(_profile_registrations(session.profile_home), session, reason)
